...
async def google_auth_callback(code: Optional[str] = None , error: Optional[str] = None, db: Session = Depends(get_db)):
    try:
        if error == "access_denied":
            return RedirectResponse(url=f"{settings.front_host}/login")
        ...
        response = RedirectResponse(url=f"{settings.front_host}/nc")
        # response = RedirectResponse(url=f"http://localhost:3000/nc")
        response.set_cookie(
